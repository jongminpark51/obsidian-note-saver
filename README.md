...
## 위젯
- `ObsidianNoteSaverWidget.cmd`를 실행하면 작은 런처 위젯이 화면 오른쪽 아래에 표시됩니다.
- 위젯은 항상 위에 표시되며, `새 메모` 버튼으로 메인 앱을 엽니다.
- 위젯 카드를 드래그해서 원하는 위치로 옮길 수 있습니다.
- 시작프로그램에 등록되어 Windows 로그인 시 자동 실행됩니다.
- 바탕화면에는 `Obsidian Note Saver Widget` 바로가기도 생성됩니다.
...
위젯 파일 상태는 다음 명령으로 확인합니다.

```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -STA -File ".\ObsidianNoteSaverWidget.ps1" -SelfTest
```
